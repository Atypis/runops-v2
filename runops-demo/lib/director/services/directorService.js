  ...
  async createNode(args, workflowId) {
    ...
    switch (type) {
      ...
      case 'group':
        ...
        break;
      case 'iterate':
        if (!config.records) throw new Error('iterate node requires "records" field - a record pattern to iterate over (e.g., "email_*", "article_*")');
        if (!config.body) throw new Error('iterate node requires "body" field - nodes to execute for each record');
        break;
      case 'context':
    ...
    try {
      ...
      switch (toolName) {
        ...
        case 'execute_nodes':
          ...
          break;
        // New container authoring tools
        case 'iterate_apply': {
          const res = await fetch(`/api/director/workflows/${encodeURIComponent(args.workflowId)}/iterate/apply`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ iterateRef: args.iterateRef, op: args.op, spec: args.spec, reorder_spec: args.reorder_spec })
          });
          result = await res.json();
          break;
        }
        case 'iterate_resolve': {
          const res = await fetch(`/api/director/workflows/${encodeURIComponent(args.workflowId)}/iterate/resolve`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ iterateRef: args.iterateRef })
          });
          result = await res.json();
          break;
        }
        case 'route_apply': {
          const res = await fetch(`/api/director/workflows/${encodeURIComponent(args.workflowId)}/route/apply`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ routeRef: args.routeRef, op: args.op, paths_spec: args.paths_spec })
          });
          result = await res.json();
          break;
        }
        case 'route_resolve': {
          const res = await fetch(`/api/director/workflows/${encodeURIComponent(args.workflowId)}/route/resolve`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ routeRef: args.routeRef })
          });
          result = await res.json();
          break;
        }
        case 'workflow_renumber_preorder': {
          const res = await fetch(`/api/director/workflows/${encodeURIComponent(args.workflowId)}/renumber_preorder`, { method: 'POST' });
          result = await res.json();
          break;
        }
