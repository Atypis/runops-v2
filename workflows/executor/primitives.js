...
class Primitives {
  ...
  async executeBrowserAction({ method, target, data }) {
    ...
    switch (method) {
      ...
      case 'type':
        const text = this.stateManager.resolveVariable(data) || data;
  ...
  executeTransform({ input, function: fn, output }) {
    ...
    if (Array.isArray(input)) {
      inputData = input.map(i => this.stateManager.resolveVariable(i));
    } else {
      inputData = this.stateManager.resolveVariable(input);
    }
  ...
  async executeCognition({ prompt, input, output, model = 'gpt-4o-mini', schema }) {
    const inputData = this.stateManager.resolveVariable(input);
...
{"investorName": "John Smith", "company": "Accel Partners"}`;
    ...
    switch (operation) {
      case 'set':
        Object.entries(data).forEach(([key, value]) => {
          ...
          const resolvedValue = this.stateManager.resolveVariable(value);
  ...
  async executeIterate({ over, as, body }) {
    ...
    const items = this.stateManager.resolveVariable(over) || [];
    const results = [];
  ...
  async executeRoute({ value, paths }) {
    const checkValue = String(this.stateManager.resolveVariable(value));
    ...
    return await this.executeNode(selectedPath);
  }
  ...
  async executeHandle({ try: tryNode, catch: catchNode, finally: finallyNode }) {
    ...
    try {
      return await this.executeNode(tryNode);
    } catch (error) {
      console.error('Caught error:', error);
      if (catchNode) {
        return await this.executeNode(catchNode);
      }
      throw error;
    } finally {
      if (finallyNode) {
        await this.executeNode(finallyNode);
      }
    }
  }
