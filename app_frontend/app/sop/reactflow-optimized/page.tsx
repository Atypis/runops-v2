...
export default function ReactFlowOptimizedSopPage() {
  ...
  useEffect(() => {
    const fetchSopData = async () => {
      ...
      setError(null);
      try {
        // Use our ReactFlow-optimized version
        const response = await fetch('/mocksop-reactflow.json');
        if (!response.ok) {
    ...
    <div className="flex flex-col h-screen bg-neutral-surface-2">
      <header className="bg-neutral-surface-1 shadow-card-default sticky top-0 z-50">
        <div className="container mx-auto h-16 flex items-center justify-between px-4">
          <p className="text-lg font-semibold text-foreground">ReactFlow-Optimized SOP Viewer</p>
          <div className="text-sm text-muted-foreground">SOP ID: mocksop-reactflow</div>
        </div>
      </header>
        ...
        <div className="flex-grow bg-white shadow-lg rounded-card-radius overflow-hidden flex flex-col">
          <div className="p-4 border-b border-neutral-surface-3">
            <h1 className="text-xl font-semibold text-foreground">
              {sopData?.meta.title || 'Optimized SOP'}
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              This SOP has been restructured to work optimally with ReactFlow's compound nodes.
            </p>
